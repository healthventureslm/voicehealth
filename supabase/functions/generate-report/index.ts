...
// Converte um JSON estruturado em markdown legível pra `content` e PDF.
// Renderização rasa: seções viram H2, fields viram listas chave: valor.
// Tabelas viram listas numeradas com sub-itens.
function structuredToMarkdown(template: TemplateSchema, filled: Record<string, unknown>): string {
  const lines: string[] = [];
...
function renderFieldMarkdown(field: { id: string; label: string; type: string; [k: string]: unknown }, value: unknown): string {
  ...
  if (field.type === "scored_scale" && typeof value === "object") {
